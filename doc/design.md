...
- 入口在 **Segments 视图标题栏的 👁 按钮**与**状态栏的 👁 项**（显示当前档位，点击开档位选择器）；当前档位常驻 **Segments 视图的 message 行**（`👁 Seg+Var · ⌶ in function` / `👁 Off · ⌶ outside function`——👁 与状态栏同款认 spotlight，⌶（I-beam）认光标字段；TreeView.message 是纯字符串、不渲染 codicon，图形只能用 unicode，几何符号 ◉/○/⊘ 实测太小且与字母难分，弃用；2026-08-11 替代活动栏数字角标——角标离 spotlight 的操作现场太远）。
...
- **View 头部两通道**：description = 在看什么（Segments=函数名、Trail=`Route: <label>`、Markers=空）；message = 状态偏离或图例（Markers=`🚫 N hidden`、Trail=徽标图例、Segments=`👁 档位 · ⌶ in/outside function`）。message 是纯字符串——不渲染 codicon，图形用 unicode/emoji（★ ↻ ↙ 👁 ⌶ 🚫）；小号几何符号（◉ ○ ⊘）在 message 字号下与字母难分，实测被否。message 非空会压制 viewsWelcome，所以"无偏离"时必须置 undefined，不许留空转态文案。
