...
- 入口在 **Segments 视图标题栏的 👁 按钮**与**状态栏的 👁 项**（显示当前档位，点击开档位选择器）；当前档位常驻 **Segments 视图的 message 行**（`◉ Seg+Var · in function` / `○ Off · outside function`——档位短名 + 光标在/不在函数内两个字段；TreeView.message 是纯字符串、不渲染 codicon，图形只能用 unicode；2026-08-11 替代活动栏数字角标——角标离 spotlight 的操作现场太远）。
...
- **View 头部两通道**：description = 在看什么（Segments=函数名、Trail=`Route: <label>`、Markers=空）；message = 状态偏离或图例（Markers=`⊘ N hidden`、Trail=徽标图例、Segments=`◉/○ 档位 · in/outside function`）。message 是纯字符串——不渲染 codicon，图形用 unicode 符号（★ ↻ ↙ ◉ ○ ⊘）；message 非空会压制 viewsWelcome，所以"无偏离"时必须置 undefined，不许留空转态文案。
...
- **空态（viewsWelcome）**：两行、零按钮：第一行本体功能 + 快捷键示例（色族只列 2 个代表 + 两个选色入口），第二行 AI 功能并指向上方按钮。welcome 里 codicon `$(icon)` 可渲染（每行行首用与标题按钮同款图标锚定指代）；**整行只有一个链接会被渲染成按钮**，故禁止整行链接。含快捷键的文案按 `isMac` / `!isMac` 写两份（⌘K vs Ctrl+K——welcome 无键位替换语法）。空态按钮是强引导，只留给明确要引导点击的场景（当前无处够格）；临时性空态走 message 一句话。
