...
| `sightread.guide.harness` | `auto` | which coding-agent CLI runs AI Interpret and route planning; `auto` probes `claude` → `codex` → `opencode` → `pi` → `cursor` → `aider` → `agy` |
