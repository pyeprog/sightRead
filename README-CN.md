...
- 🗂️ **侧边栏** —— SightRead 活动栏容器包含三个视图：**Entry Points**（从哪里开始读这个文件）、**Segments**（当前函数的段落树）和 **Markers**（工作区内所有荧光标记）。
...
| `SightRead: Mark Selection…` | 给选区打荧光标记，可选颜色和备注 |
| `SightRead: Mark Selection Yellow (key point)` | 一键黄色标记（重点） |
| `SightRead: Mark Selection Red (doubt)` | 一键红色标记（疑问） |
| `SightRead: Mark Selection Green (verified)` | 一键绿色标记（已确认） |
...
| `SightRead: Cycle Spotlight Level` | 按 Off → Seg+Var → Seg → Fn 循环，等同点击状态栏的 👁 |
| `SightRead: Spotlight Off` | 直接关闭聚光灯 |
