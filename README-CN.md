...
- **聚光灯** —— 排除其他函数、其他无关代码块的视觉干扰。点击状态栏的 👁 按钮按 Off → Seg+Var → Seg → Fn 循环切换。
  ...
  4. **Off** —— 关闭聚光灯，默认模式。
- **自动分代码块** —— 按空行 + 关键词把函数切成**递归结构**，方便在segment窗口展示函数大结构，点击可以跳转到相应代码块。节点旁还会以灰色小字显示压缩后的条件/表达式（悬停可看完整首行）。
- **侧边栏** —— SightRead 活动栏容器包含两个视图：**Segments**（当前函数的段落树）和 **Markers**（工作区内所有荧光标记）。
