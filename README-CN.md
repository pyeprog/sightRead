...
| `SightRead: Plan Reading Route (AI)` | 输入阅读目标，agent 探索仓库并把规划路线（暗色）种进 Trail 视图 |
| `SightRead: Trace Entries to Current Code (AI)` | 列出所有能到达光标处代码的入口，铺成 Trail 视图里的链 |
| `SightRead: Clear Routes` | 清除仍处于规划态的跳与全部路线徽章 |
| `SightRead: Go to Caller Site` | （右键 trail 节点）跳到调用方里调用它的那一行 |
