...
import { condenseHeader, scanSignatureEnd, shortenPath, statementSummary } from './expression';
...
export function classify(top: string[], opts: SegmentationOptions): Summary {
  ...
  // Function-valued binding (`const f = () => {…}`) — a definition in
  // disguise, same carve-out as the tsJs module. Only when the binding owns
  // its unit: alone, or followed by nothing but closers/continuations.
