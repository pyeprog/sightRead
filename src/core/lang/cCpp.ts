/**
 * C / C++ syntax — one module for both, C++ being the superset that matters
 * here (`try`/`catch` without finally, `class`/`namespace`, constructor
 * initializer lists starting with `:`). Preprocessor lines (`#…`) already
 * read as comment-leads and bind to the segment that follows.
 */
...
const SINGLE_KW_RE = /^(if|for|while|do|switch|try|return|throw|goto|break|continue)\b/;
const DEFINITION_RE =
  /^(?:template\s*<[^>]*>\s*)?(class|struct|enum|union|namespace)\s+([A-Za-z_]\w*)?/;
const FLOW_KEYWORDS = new Set(['return', 'throw', 'goto', 'break', 'continue']);
...
/** `try`, `try/catch`, `try/catch{2}` — C++ has no finally. */
...
export const cCppSyntax: LanguageSyntax = {
  isContinuationLine,
  isKeywordLead,
  classify,
  findBodyStart,
