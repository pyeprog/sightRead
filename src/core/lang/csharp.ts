/**
 * C# syntax. `using` and `lock` are scope statements (with kind); `foreach`
 * joins the loop family. `using` is a hard keyword here — no lookahead needed.
 */
...
const SINGLE_KW_RE =
  /^(if|for|foreach|while|do|switch|try|using|lock|return|throw|yield|break|continue)\b/;
const DEFINITION_RE =
  /^(?:(?:public|private|protected|internal|static|abstract|sealed|partial|readonly|async)\s+)*(class|interface|struct|enum|record)\s+([A-Za-z_]\w*)?/;
