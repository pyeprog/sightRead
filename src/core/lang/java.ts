/**
 * Java syntax. `synchronized (lock) { … }` reads as a scope statement, so it
 * takes the with kind; `yield` is the switch-expression flow word (Java 14+).
 */
...
const SINGLE_KW_RE =
  /^(if|for|while|do|switch|try|synchronized|return|throw|yield|break|continue)\b/;
const DEFINITION_RE =
  /^(?:(?:public|private|protected|static|abstract|final|sealed|strictfp)\s+)*(class|interface|enum|record)\s+([A-Za-z_$][\w$]*)?/;
