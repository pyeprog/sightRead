/**
 * Kotlin syntax. `when` is the dispatch statement; `enum`/`data`/`companion`
 * read as modifiers so `enum class` / `data class` / `companion object` name
 * the class or object they define.
 */
...
const CONTINUATION_RE =
  /^(\}|\)|\]|\{|\.|\?|&&|\|\||else\b|catch\b|finally\b)/;
const SINGLE_KW_RE = /^(if|for|while|do|when|try|return|throw|break|continue)\b/;
const DEFINITION_RE =
  /^(?:(?:public|private|internal|protected|open|abstract|final|sealed|data|enum|inner|companion|suspend|override|operator|inline|tailrec)\s+)*(fun|class|interface|object)\s+([A-Za-z_]\w*)?/;
const FLOW_KEYWORDS = new Set(['return', 'throw', 'break', 'continue']);
...
export const kotlinSyntax: LanguageSyntax = {
  isContinuationLine,
  isKeywordLead,
  classify,
  findBodyStart,
